...
import DepreciationsTable from "components/DepreciationsTable/DepreciationsTable.tsx";
import {useNavigate} from "react-router-dom";

const DepreciationsPage = () => {

    const dispatch = useAppDispatch()

    const navigate = useNavigate()

    const depreciations = useAppSelector((state) => state.depreciations.depreciations)

    const filters = useAppSelector<T_DepreciationsFilters>((state) => state.depreciations.filters)
    ...
    useEffect(() => {
        dispatch(fetchDepreciations())
    }, [filters]);

    const applyFilters = async (e: FormEvent) => {
        e.preventDefault()

        const filters:T_DepreciationsFilters = {
            status: status,
    ...
    return (
        <Container>
            ...
            {depreciations.length ? <DepreciationsTable depreciations={depreciations}/> : <h3 className="text-center mt-5">Список пуст</h3>}
        </Container>
    )
};
