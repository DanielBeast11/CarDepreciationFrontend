...
import {AsyncThunkConfig, E_DepreciationStatus, T_CartDataResponse, T_CarMMUpdate} from "modules/types"
...
export const fetchDepreciations = createAsyncThunk<Depreciations[], void, AsyncThunkConfig>(
    ...
    async function(_, thunkAPI) {
        ...
        return response.data.filter(depreciation => depreciation.owner?.includes(state.depreciations.filters.owner))
    }
)
...
export const updateCarValue = createAsyncThunk<void, T_CarMMUpdate, AsyncThunkConfig>(
...
export const acceptDepreciation = createAsyncThunk<void, string, AsyncThunkConfig>(
    "depreciations/accept_depreciation",
    async function(depreciation_id, thunkAPI) {
        await api.depreciations.depreciationsUpdateStatusAdminUpdate(depreciation_id, {status: E_DepreciationStatus.Completed})
        thunkAPI.dispatch(fetchDepreciations())
    }
)

export const rejectDepreciation = createAsyncThunk<void, string, AsyncThunkConfig>(
    "depreciations/accept_depreciation",
    async function(depreciation_id, thunkAPI) {
        await api.depreciations.depreciationsUpdateStatusAdminUpdate(depreciation_id, {status: E_DepreciationStatus.Rejected})
        thunkAPI.dispatch(fetchDepreciations())
    }
)
